...
import UIKit
import Firebase
class ViewProfileViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UITableViewDelegate, UITableViewDataSource{
    var y = 0.0
    var counter = 0
    let overlay = UIView()
                ...
                for events in snapshot.children.allObjects as! [DataSnapshot]{
                        if(events.key == "Interested Users"){
                            ...
                            for users in events.children.allObjects as! [DataSnapshot]{
                                if(users.value as? String == user.userID){
    ...
    func fetchEvents() {
        ...
        Database.database().reference().child("Events").observe(.childAdded, with: { (snapshot) in
            if let dict = snapshot.value as! [String: AnyObject]?
            {
                if(dict["Host"] as? String == user.userID){
                    let event = Event()
                    event.eventName = dict["Event Name"] as? String
                    event.codedDate = dict["Coded Date"] as? String
                    event.location = dict["Event Location"] as? String
    ...
    func fetchInterests() {
        ...
        Database.database().reference().child("Users").child(user.userID).observe(.childAdded, with: { (snapshot) in
    ...
    override func viewDidLoad() {
        ...
        ref.child("Users").child(user.userID).observeSingleEvent(of: .value, with: {(snapshot)
            in
            if let dict = snapshot.value as? [String: AnyObject]
            {
        ...
        var firstName = ""
        var lastName = ""
        Database.database().reference().child("Users").child(user.userID).observeSingleEvent(of: .value, with: {(snapshot)
            in
            if let dict = snapshot.value as? [String: AnyObject]
            {
